...
import { EEuSelectType } from './eu-select.schematics';
import { EventManager } from '@angular/platform-browser';

@Component({
  selector: 'eu-select',
  templateUrl: './eu-select.component.html',
  styleUrls: ['./eu-select.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class EuSelectComponent implements OnInit {
  ...
  @Input() type: string = EEuSelectType.default;
  ...
  @HostListener('document:keydown', ['$event']) charKeyDownHandler(
    ...
    if (this.focused) {
      ...
      if (key.length == 1 && key.match(/[a-z]/i)) {
        let chosenItemIndex;
        if (this.type == EEuSelectType.default) {
  ...
  ngOnInit(): void {
    ...
    if (this.emptyOption) {
      this.myItems = [
        this.type == EEuSelectType.default ? { name: '', value: '' } : '',
        ...this.items,
      ];
    } else {
  ...
  get optionsClass() {
    return this.focused ? '' : 'eu-select-hidden';
  }
  ...
  change(value) {
    if (value != null && value != undefined) {
      this.inputValue = this.type == EEuSelectType.pure ? value : value.name;
    }
  ...
  getDisplayName(item) {
    return this.type == EEuSelectType.pure ? item : item.name;
  }
}
